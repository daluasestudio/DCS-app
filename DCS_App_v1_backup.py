...
<style>
    ...
    /* Center all Streamlit images (helps center login logo) */
...
def authenticate(username, password):
    users_df = load_users()
    user = users_df[(users_df["username"] == username) & (users_df["status"] == "active")]
    if not user.empty:
        stored_hash = user.iloc[0]["password_hash"]
        if hash_password(password) == stored_hash:
            role = user.iloc[0]["role"]
            features = user.iloc[0].get("features", "")
            capabilities = user.iloc[0].get("capabilities", "")
            return True, role, features, capabilities
    return False, None, "", ""
